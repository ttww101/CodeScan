...
@property (nonatomic, assign) MMScanType scanTmpType;
@end
...
- (void)initScanType{
    if (self.scanType == MMScanTypeAll) {
        ...
        _scanRect = CGRectFromString([self scanRectWithScale:1][0]);
        self.output.rectOfInterest = _scanRect;
        _tipTitle.text = NSLocalizedString(@"orscan.items.scanORCode.alert", "");
        ...
        _tipTitle.text = NSLocalizedString(@"orscan.items.scanBarCode.alert", "");
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    {
        [self showError:NSLocalizedString(@"orscan.items.scanORCodeFail.alert", "")];
        return;
    }
    ...
    if (self.scanFinish) {
        ...
        if (self.navigationController &&[self.navigationController respondsToSelector:@selector(popViewControllerAnimated:)]) {
            ...
            if (self.scanType == MMScanTypeAll) {
                ...
                if (self.scanTmpType == MMScanTypeQrCode) {
                    
                    url = [NSString stringWithFormat:@"Q>%@", url];
                    
                }else if (self.scanTmpType == MMScanTypeBarCode){
                    
                    url = [NSString stringWithFormat:@"B>%@", url];
                }
            }
...
- (void)drawBottomItems
{
    ...
    [_scanTypeQrBtn setTitle:NSLocalizedString(@"orscan.items.ORCode", "") forState:UIControlStateNormal];
    ...
    [_scanTypeBarBtn setTitle:NSLocalizedString(@"orscan.items.BarCode", "") forState:UIControlStateNormal];
...
- (void)setNavItem:(MMScanType)type {
    if(type == MMScanTypeBarCode) {
        ...
        UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"orscan.items.scanAlbum", "") style:UIBarButtonItemStylePlain target:self action:@selector(openPhoto)];
...
- (void)qrBtnClicked:(UIButton *)sender {
    ...
    delayQRAction = YES;
    self.scanTmpType = MMScanTypeQrCode;
...
- (void)barBtnClicked:(UIButton *)sender {
    ...
    delayBarAction = YES;
    self.scanTmpType = MMScanTypeBarCode;
    [self performTaskWithTimeInterval:3.0f action:^{
        delayBarAction = NO;
    }];
}
...
- (void)changeScanCodeType:(MMScanType)type {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        _tipTitle.text = type == MMScanTypeQrCode ?NSLocalizedString(@"orscan.items.scanORCode.alert", "") : NSLocalizedString(@"orscan.items.scanBarCode.alert", "");
...
- (void)openPhoto
{
    ...
    {
        NSString *tipMessage = [NSString stringWithFormat:NSLocalizedString(@"orscan.items.scanSysSetting", ""),appName];
        [self showError:tipMessage andTitle:NSLocalizedString(@"orscan.items.scanLibaryFail", "")];
    }
}
...
- (BOOL)isAvailableCamera {
    if([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera])
    {
        ...
        if (authorizationStatus == AVAuthorizationStatusRestricted ||
            ...
            NSString *tipMessage = [NSString stringWithFormat:NSLocalizedString(@"orscan.items.scanSysSetting", "") ,appName];
            [self showError:tipMessage andTitle:NSLocalizedString(@"orscan.items.scanCarmerFail", "")];
...
- (void)showError:(NSString*)str {
    [self showError:str andTitle:NSLocalizedString(@"gif.base.alert.alert", "")];
}
